...
class Edrone():
	...
	#	8. Add error_sum
		...
		def pitch_pid():
			...
			self.cmd.rcPitch=int(1500-(self.pitch_error*self.Kp[1]+(self.pitch_error-self.pitch_prev_error)*self.Kd[1]+ self.pitch_sum_error*self.Ki[1])/self.sample_time)
		...
		def roll_pid():
			...
			self.cmd.rcRoll=int(1500+(self.roll_error*self.Kp[0]+(self.roll_error-self.roll_prev_error)*self.Kd[0]+ self.roll_sum_error*self.Ki[0])/self.sample_time)
